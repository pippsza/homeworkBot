...
const { syncInfoChunks } = require("../../services/infoChunkingService");
...
router.post("/:id/rechunk", requireSuperadmin, async (req, res) => {
  const info = await infoService.getById(req.params.id);
  if (!info) return res.status(404).json({ error: "Not found" });
  try {
    await syncInfoChunks(info._id);
    const updated = await infoService.getById(info._id);
