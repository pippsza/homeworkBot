...
const GOOGLE_MODELS: GoogleModelEntry[] = [
  {
...
async function syncGoogleModels(): Promise<number> {
  let upserted = 0;
...
async function syncOpenRouterModels(): Promise<number> {
...
async function syncAll(): Promise<{ google: number; openrouter: number }> {
...
async function getAll(filters: ModelCatalogFilters = {}): Promise<any[]> {
...
async function getByProviderAndId(provider: string, modelId: string): Promise<any> {
