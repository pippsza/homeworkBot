 ...
 */
function createLanguageModel(provider, modelId) {
  if (provider === "google") {
 ...
 */
async function getModelConfigCached() {
  const now = Date.now();
 ...
 */
async function resolveModels(role) {
  ...
  // Primary
  const primaryProvider = roleSettings.provider || roleConfig.default.provider;
  const primaryModelId = roleSettings.modelId || roleConfig.default.modelId;
...
async function getSolveModels(hasImages = false) {
  return resolveModels(hasImages ? "solveImage" : "solveText");
}

async function getProSolveModels(hasImages = false) {
