...
function subjectsHandler(bot) {
  ...
  function subjectsHandler(bot) {
    ...
    bot.action(/delete_subject_(\d+)/, async (ctx) => {
      if (!isAdmin(ctx)) return ctx.reply("Нет прав.");
      if (!ctx.match || typeof ctx.match[1] === "undefined")
        return ctx.reply("Ошибка callback.");
      const idx = Number(ctx.match[1]);
      if (!data.subjects || !data.subjects[idx])
        return ctx.reply("Предмет не найден.");
      const removed = data.subjects.splice(idx, 1);
      saveData(data);
      // После удаления показываем актуальное меню предметов
      let msg = "Список предметов:\n";
      data.subjects.forEach((s, i) => {
        const emoji = data.subjectEmojis[s.name] || "📚";
        msg += `${emoji} ${s.name}\n`;
    ...
    bot.action(/delete_task_(\d+)_(\d+)/, async (ctx) => {
      ...
      if (
        !ctx.match ||
        typeof ctx.match[1] === "undefined" ||
        typeof ctx.match[2] === "undefined"
      )
        return ctx.reply("Ошибка callback.");
      const sIdx = Number(ctx.match[1]);
      const tIdx = Number(ctx.match[2]);
      ...
      saveData(data);
      // После удаления показываем актуальное меню задач для предмета
      const subject = data.subjects[sIdx];
      if (!subject) {
        await editOrSend(
          ctx,
          `Задание ${removed[0]?.title || "?"} удалено.\n\nПредмет не найден.`,
          Markup.inlineKeyboard([
            [Markup.button.callback("⬅ Назад", "subjects")],
          ])
        );
      ...
      // Формируем кнопки заданий по 3 в ряд
      const taskButtons = [];
      for (let j = 0; j < subject.tasks.length; j += 3) {
        taskButtons.push(
          subject.tasks
    ...
    bot.action(/viewers_remove_(\d+)/, async (ctx) => {
      ...
      await refreshAnswerViewers(ctx);
      console.log(
        `[LOG] ${ctx.from.username} удалил ANSWER_VIEWER: ${removed}`
      );
    });
    ...
    bot.action("superusers_add", async (ctx) => {
      ...
      console.log(
        `[LOG] ${ctx.from.username} начал добавление нового SUPERUSER`
      );
    });
    ...
    bot.on("text", async (ctx) => {
      ...
      if (taskState && taskState.step === "add_superuser") {
        ...
        await showSuperusersMenu(ctx);
        console.log(
          `[LOG] ${ctx.from.username} добавил SUPERUSER: ${username}`
        );
        return;
      }
    });
  ...
  bot.action(/delete_task_(\d+)_(\d+)/, async (ctx) => {
    ...
    // Перезагружаем данные для обеспечения актуальности
    ...
    // Обновляем ID сообщения для будущих редактирований
    updateInteractiveMessageId(ctx, newMessage.message_id);

    console.log(`[LOG] ${ctx.from.username} удалил задание: ${taskTitle}`);
  });

  // Добавляем функцию для обновления ID сообщения
  function updateInteractiveMessageId(ctx, messageId) {
    if (ctx.chat && ctx.chat.id) {
      interactiveMessageId[ctx.chat.id] = messageId;
    }
  }
